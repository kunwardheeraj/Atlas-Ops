...
// ─── Report Processor ─────────────────────────────────────────────────────────
async function processReportJob(bullJob) {
    ...
    console.log(`[reportWorker] Processing job ${data.jobNumber} | ` +
...
`;
    ...
    console.log(`[reportWorker] ✅ Report generated for ${data.jobNumber} | ` +
        `Model: ${MODEL_NAME} | Flagged issues: ${parsed.flaggedIssues.length}`);
    return parsed;
}
// ─── Worker Initialization ────────────────────────────────────────────────────
export function startReportWorker() {
    const worker = new Worker(QUEUE_NAME, processReportJob, {
        connection: {
    ...
    worker.on('failed', (job, err) => {
        ...
        if (attemptsMade >= maxAttempts) {
            ...
            console.warn(`[reportWorker] ⚠️  Job ${job?.id} failed (attempt ${attemptsMade}/${maxAttempts}). ` +
