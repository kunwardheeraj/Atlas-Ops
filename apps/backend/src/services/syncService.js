...
function resolveStructuredField(field, serverValue, clientValue, serverUpdatedAt, clientUpdatedAt, conflicts) {
...
function resolveNotes(serverNotes, clientNotes, conflicts) {
    // No divergence — no conflict
    if (serverNotes === clientNotes)
        return serverNotes;
...
function resolveChecklist(serverItems, clientItems, conflicts) {
...
// ─── Main Resolver ────────────────────────────────────────────────────────────
export function resolveJobConflict(serverJob, clientJob) {
    const conflicts = [];
    ...
    const resolvedStatus = resolveStructuredField('status', serverJob.status, clientJob.status, serverJob.updatedAt, clientJob.updatedAt, conflicts);
    const resolvedTitle = resolveStructuredField('title', serverJob.title, clientJob.title, serverJob.updatedAt, clientJob.updatedAt, conflicts);
    const resolvedPriority = resolveStructuredField('priority', serverJob.priority, clientJob.priority, serverJob.updatedAt, clientJob.updatedAt, conflicts);
    const resolvedLocation = resolveStructuredField('location', serverJob.location, clientJob.location, serverJob.updatedAt, clientJob.updatedAt, conflicts);
    // ── 2. Notes: Server-Wins + safe append ────────────────────────────────────
    const resolvedNotes = resolveNotes(serverJob.notes, clientJob.notes, conflicts);
    // ── 3. Checklist: Union-Merge (Add-Wins) ───────────────────────────────────
    const resolvedChecklist = resolveChecklist(serverJob.checklist, clientJob.checklist, conflicts);
    // ── Determine overall strategy ─────────────────────────────────────────────
    let strategy = 'no_conflict';
