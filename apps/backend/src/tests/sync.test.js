...
import { resolveJobConflict } from '../services/syncService.js';
// ─── ANSI color helpers ───────────────────────────────────────────────────────
...
test('Notes: server notes are fully preserved in resolved output', () => {
    assert.ok(resolved.notes?.includes('Initial inspection complete.'), `Server notes not found in resolved notes: "${resolved.notes}"`);
});
test('Notes: client notes are appended with [CONFLICT] flag', () => {
    assert.ok(resolved.notes?.includes('[CONFLICT: Client added:'), `[CONFLICT] flag not found in resolved notes: "${resolved.notes}"`);
});
test('Notes: client notes content is not lost', () => {
    assert.ok(resolved.notes?.includes('unusual vibration on compressor unit B'), `Client note content was silently dropped: "${resolved.notes}"`);
});
console.log();
...
test('Checklist: no duplicate items (union deduplication)', () => {
    ...
    assert.equal(resolved.checklist.length, uniqueCount, `Duplicates found: ${resolved.checklist.join(', ')}`);
});
